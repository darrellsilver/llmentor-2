...
function getMergedPipeline(pipeline: Pipeline, properties: PipelineProperty[]) {
  ...
  for (const property of properties) {
    const pipelineNode = mergedPipeline.nodes.find(
      (node) => node.type === property.type && node.id === property.id
    );
    ...
    switch (pipelineNode.type) {
      ...
      case NodeType.TranscriptNode:
        pipelineNode.transcriptId = (
          property as TranscriptProperty
        ).transcriptId;
        break;
    }
  }
...
async function runOutputNode(
  ...
  const inputReferences =
    node.inputReferences || (node.inputReference ? [node.inputReference] : []);
  const inputNodes = await Promise.all(
    inputReferences.map(async (inputReference) => await getNode(inputReference))
  );
...
async function runOpenAiNode(
  ...
  // Sort by y position here (top to bottom)
  // TODO Move to client (currently only references are available there)
  const sortedContextNodes = contextNodes.sort(
    (a, b) => a.position.y - b.position.y
  );

  const contextResults = await Promise.all(
    sortedContextNodes.map(
  ...
  const modelName =
    totalLength / 4 > 3500 ? "gpt-3.5-turbo-16k" : "gpt-3.5-turbo";
...
async function getNode(nodeRef: NodeReference): Promise<PipelineNode> {
  // Worst database ever
  return currentPipeline.nodes.find(
    (n) => nodeRef.type === n.type && nodeRef.id === n.id
  ) as PipelineNode;
}
